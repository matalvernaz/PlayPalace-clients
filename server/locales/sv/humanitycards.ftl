...
# Judge announcement
hc-judge-is = { $count ->
    [one] { $names } är korttsaren.
   *[other] { $names } är korttsarerna.
}
...
# Results
hc-winner-announcement = { $player } får { $points } { $points ->
    [one] poäng
   *[other] poäng
} för { $text }.
...
hc-waiting-for = Väntar på att { $names } ska lämna in.
hc-all-submitted-waiting-judge = Alla spelare har lämnat in. Väntar på att { $judge } ska döma.
hc-waiting-for-judges = Väntar på att { $names } ska döma.
