...
# Judge announcement
hc-judge-is = { $count ->
    [one] { $names } je car kart.
   *[other] { $names } so carji kart.
}
...
# Results
hc-winner-announcement = { $player } dobi { $points } { $points ->
    [one] točko
   *[other] točke
} za { $text }.
...
hc-waiting-for = Čaka se na { $names }, da oddajo karte.
hc-all-submitted-waiting-judge = Vsi igralci so oddali karte. Čaka se na odločitev od { $judge }.
hc-waiting-for-judges = Čaka se na odločitev od { $names }.
