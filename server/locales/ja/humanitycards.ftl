...
# Results
hc-winner-announcement = { $player } は { $text } で { $points } { $points ->
    [one] 点
   *[other] 点
} を獲得します。
...
hc-waiting-for = { $names } の提出を待っています。
hc-all-submitted-waiting-judge = 全員が提出しました。{ $judge } の判定を待っています。
hc-waiting-for-judges = { $names } の判定を待っています。
