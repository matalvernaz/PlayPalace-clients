...
# Judge announcement
hc-judge-is = { $count ->
    [one] { $names } jest carem kart.
   *[other] { $names } są carami kart.
}
...
# Results
hc-winner-announcement = { $player } zdobywa { $points } { $points ->
    [one] punkt
   *[other] punkty
} za { $text }.
...
hc-waiting-for = Oczekiwanie na ruch od { $names }.
hc-all-submitted-waiting-judge = Wszyscy gracze już zagrali. Oczekiwanie na ocenę od { $judge }.
hc-waiting-for-judges = Oczekiwanie na ocenę od { $names }.
