...
# Judge announcement
hc-judge-is = { $count ->
    [one] { $names } — карточный царь.
   *[other] { $names } — карточные цари.
}
...
# Results
hc-winner-announcement = { $player } получает { $points } { $points ->
    [one] очко
   *[other] очков
} за { $text }.
...
hc-waiting-for = Ожидание, пока { $names } сдадут карты.
hc-all-submitted-waiting-judge = Все игроки уже сдали карты. Ожидание решения от { $judge }.
hc-waiting-for-judges = Ожидание решения от { $names }.
