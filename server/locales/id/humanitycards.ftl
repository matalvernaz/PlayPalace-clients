...
# Judge announcement
hc-judge-is = { $count ->
    [one] { $names } adalah Card Czar.
   *[other] { $names } adalah para Card Czar.
}
...
# Results
hc-winner-announcement = { $player } mendapat { $points } { $points ->
    [one] poin
   *[other] poin
} untuk { $text }.
...
hc-waiting-for = Menunggu { $names } mengirimkan kartu.
hc-all-submitted-waiting-judge = Semua pemain sudah mengirimkan kartu. Menunggu { $judge } untuk menilai.
hc-waiting-for-judges = Menunggu { $names } untuk menilai.
