...
# Judge announcement
hc-judge-is = { $count ->
    [one] { $names } is de kaarttsaar.
   *[other] { $names } zijn de kaarttsaren.
}
...
# Results
hc-winner-announcement = { $player } krijgt { $points } { $points ->
    [one] punt
   *[other] punten
} voor { $text }.
...
hc-waiting-for = Wachten op { $names } om in te dienen.
hc-all-submitted-waiting-judge = Alle spelers hebben ingezonden. Wachten op het oordeel van { $judge }.
hc-waiting-for-judges = Wachten op het oordeel van { $names }.
