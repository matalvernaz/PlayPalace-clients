...
# Judge announcement
hc-judge-is = { $count ->
    [one] { $names } là Sa hoàng thẻ bài.
   *[other] { $names } là các Sa hoàng thẻ bài.
}
...
# Results
hc-winner-announcement = { $player } nhận được { $points } { $points ->
    [one] điểm
   *[other] điểm
} cho { $text }.
...
hc-all-submitted-waiting-judge = Tất cả người chơi đã nộp bài. Đang chờ { $judge } chấm.
hc-waiting-for-judges = Đang chờ { $names } chấm.
