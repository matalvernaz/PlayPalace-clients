...
# Judge announcement
hc-judge-is = { $count ->
    [one] { $names } a kártyacár.
   *[other] { $names } a kártyacárok.
}
...
# Results
hc-winner-announcement = { $player } { $points } { $points ->
    [one] pontot
   *[other] pontot
} kap ezért: { $text }.
...
hc-waiting-for = Várakozás erre: { $names }, hogy beküldjék a lapjaikat.
hc-all-submitted-waiting-judge = Minden játékos beküldte a lapjait. Várakozás erre: { $judge }, hogy bíráljon.
hc-waiting-for-judges = Várakozás erre: { $names }, hogy bíráljanak.
