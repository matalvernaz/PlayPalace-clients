...
# Judge announcement
hc-judge-is = { $count ->
    [one] { $names } est le tsar des cartes.
   *[other] { $names } sont les tsars des cartes.
}
...
# Results
hc-winner-announcement = { $player } obtient { $points } { $points ->
    [one] point
   *[other] points
} pour { $text }.
...
hc-waiting-for = En attente de la soumission de { $names }.
hc-all-submitted-waiting-judge = Tous les joueurs ont soumis leurs cartes. En attente du jugement de { $judge }.
hc-waiting-for-judges = En attente du jugement de { $names }.
