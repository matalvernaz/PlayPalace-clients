...
# Judge announcement
hc-judge-is = { $count ->
    [one] { $names } je karetní car.
   *[other] { $names } jsou karetní caři.
}
...
# Results
hc-winner-announcement = { $player } získává { $points } { $points ->
    [one] bod
   *[other] body
} za { $text }.
...
hc-waiting-for = Čeká se na { $names }, až odevzdají karty.
hc-all-submitted-waiting-judge = Všichni hráči odevzdali karty. Čeká se na rozhodnutí od { $judge }.
hc-waiting-for-judges = Čeká se na rozhodnutí od { $names }.
