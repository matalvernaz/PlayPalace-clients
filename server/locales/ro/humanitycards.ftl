...
# Judge announcement
hc-judge-is = { $count ->
    [one] { $names } este țarul cărților.
   *[other] { $names } sunt țarii cărților.
}
...
# Results
hc-winner-announcement = { $player } primește { $points } { $points ->
    [one] punct
   *[other] puncte
} pentru { $text }.
...
hc-waiting-for = Se așteaptă ca { $names } să trimită cărțile.
hc-all-submitted-waiting-judge = Toți jucătorii au trimis cărțile. Se așteaptă verdictul lui { $judge }.
hc-waiting-for-judges = Se așteaptă verdictul lui { $names }.
