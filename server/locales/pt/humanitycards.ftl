...
# Judge announcement
hc-judge-is = { $count ->
    [one] { $names } é o czar das cartas.
   *[other] { $names } são os czares das cartas.
}
...
# Results
hc-winner-announcement = { $player } recebe { $points } { $points ->
    [one] ponto
   *[other] pontos
} por { $text }.
...
hc-waiting-for = Aguardando { $names } enviarem as cartas.
hc-all-submitted-waiting-judge = Todos os jogadores enviaram suas cartas. Aguardando o julgamento de { $judge }.
hc-waiting-for-judges = Aguardando o julgamento de { $names }.
