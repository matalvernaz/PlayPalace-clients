...
# Judge announcement
hc-judge-is = { $count ->
    [one] { $names } je car karata.
   *[other] { $names } su carevi karata.
}
...
# Results
hc-winner-announcement = { $player } dobiva { $points } { $points ->
    [one] bod
   *[other] bodova
} za { $text }.
...
hc-waiting-for = Čeka se da { $names } predaju karte.
hc-all-submitted-waiting-judge = Svi igrači su predali karte. Čeka se da { $judge } presudi.
